...
from .env_utils import load_data
...
class EnvironmentNew(object):
    ...
    def load_topology_object(self):
        ...
        path = os.path.join(self.base_dir, self.network, f'{self.network}.npz')
        data = np.load(path)
    ...
    def step_back(self, action):
        state, reward, info = self.step(action, step_back=True)
        return state, reward, info
    ...
    ****************************************************************************
    ...
    def _distribute_link_traffic(self, routing=None):
        ...
        if self.routing == 'sp':
            ...
            visited_pairs = set()
            self.next_hop_dict = {i: {j: set() for j in range(self.G.number_of_nodes() - 1) if j != i} for i in
